...
if st.button("🔮 วิเคราะห์และคำนวณราคา"):
    if model_pipeline is None:
        st.error("⚠️ ไม่พบไฟล์ 'model.joblib' กรุณาตรวจสอบไฟล์โมเดล")
    else:
...
# --- 5. ปัจจัยสำคัญ (Feature Importance - แก้ไขส่วนที่ผิด) ---
if show_importance and model_pipeline is not None:
    ...
    try:
        ...
        st.bar_chart(top_10)
        st.info("💡 คำที่มี 'name_' มาจากชื่อสินค้า ส่วน 'desc_' มาจากคำอธิบาย")
    except:
        st.warning("ไม่สามารถดึงค่าความสำคัญจากโมเดลปัจจุบันได้ (อาจเป็นเพราะโมเดลบางประเภทไม่รองรับ Feature Importance)")

st.divider()
st.caption("พัฒนาโดย: [จิรกิตติ์ บังเกิดผล] | 67160320 |")